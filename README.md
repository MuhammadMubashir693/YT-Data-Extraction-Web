...
- `GET /api/channel-videos?channelId=...&mode=keyword|date&keyword=...&keywordTitle=...&keywordDescription=...&keywordChannel=...&startDate=YYYY-MM-DD&endDate=YYYY-MM-DD&durationFilter=short|medium|long&matchMode=every|some&maxResults=<1-500>&sort=...` — search/filter videos within a specific channel. Legacy — the Search tab now uses `/api/search-videos` with `channelId` for this instead.
- `GET /api/search-videos?channelId=...&keyword=...&keywordTitle=...&keywordDescription=...&keywordChannel=...&startDate=YYYY-MM-DD&endDate=YYYY-MM-DD&durationFilter=short|medium|long&matchMode=every|some&maxResults=<1-500>&sort=...` — video search across YouTube, optionally scoped to a channel via `channelId` (the Search tab uses this for both "search within a channel" and general search — `channelId` is optional, and on its own counts as a valid filter).
...
- **Comment Picker** – fetches one page of top‑level comments at a time (latest‑first) and randomly selects one thread to display together with all its replies (automatically loading all reply pages). The tab provides Next Page and Previous Page buttons; each page is cached client‑side so returning to a previously visited page is instant. The total comment count (if available from video stats) is shown at the top.
